function python_load_editor(editor_div,
    ...
    if (document.getElementById(editor_div) != null && triggered_id != null) {
        ...
        editor_element_index = triggered_id["index"];
        editor.setValue(code[editor_element_index] ?? "");
        editor.setTheme("ace/theme/monokai");
        editor.session.setMode("ace/mode/python");
        return [editor_element_index, "Editing code for: " + button_value[editor_element_index]];
    } else {
        return [0, ""];
    }
}
...
function python_save_code_by_index(editor_div,
    n_clicks,
    editor_element_index, code) {
    // if (n_clicks == 0) {
    //     return null;
    // }
    editor_element_index = editor_element_index;
    code[editor_element_index] = ace.edit(editor_div).getValue();
    return [editor_element_index, code];
}